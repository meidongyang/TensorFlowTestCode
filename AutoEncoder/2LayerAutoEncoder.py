...
'''只把1LayerAutoEncoder稍微修改一下，在编码和解码中各加一层.
然后添加一些TensorBoard汇总节点.

我试了几组参数，效果都不如1层的
                  1    2    3    4    5
n_hidden1_units  512  500  256  128  256
n_hidden2_units  256  150  256  128  128

用的激活函数有：ReLU、Sigmoid、tanh、softplus四种
（ tanh效果最差，loss最高，重构的图像都是空白 ）
'''
...
# 网络模型参数
n_hidden1_units = 500  # 隐藏层1输入神经元数量（编码器和解码器都有相同规模的数量）
n_hidden2_units = 150  # 隐藏层2输入神经元数量（编码器和解码器都有相同规模的数量）
...
def Encoder(x_origin, activate_func=tf.nn.sigmoid):
    ...
    with tf.name_scope('Encoder_Layer1'):
        weight = WeightsVariable(n_input_units, n_hidden1_units, 'Weight')
        bias = BiasesVariable(n_hidden1_units, 'Bias')
        x_code1 = activate_func(tf.add(tf.matmul(x_origin, weight), bias))
    # 编码器hidden2
    with tf.name_scope('Encoder_Layer2'):
        weight = WeightsVariable(n_hidden1_units, n_hidden2_units, 'Weight')
        bias = BiasesVariable(n_hidden2_units, 'Bias')
        x_code2 = activate_func(tf.add(tf.matmul(x_code1, weight), bias))
    return x_code2
...
def Decoder(x_code, activate_func=tf.nn.sigmoid):
    ...
    with tf.name_scope('Decoder_Layer1'):
        weight = WeightsVariable(n_hidden2_units, n_hidden1_units, 'Weight')
        bias = BiasesVariable(n_hidden1_units, 'Bias')
        x_decode1 = activate_func(tf.add(tf.matmul(x_code, weight), bias))
    # 解码器hidden2
    with tf.name_scope('Decoder_Layer2'):
        weight = WeightsVariable(n_hidden1_units, n_output_units, 'Weight')
        bias = BiasesVariable(n_output_units, 'Bias')
        x_decode2 = activate_func(tf.add(tf.matmul(x_decode1, weight), bias))
    return x_decode2
