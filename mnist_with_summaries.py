...
def train():
    ...
    # 创建一个多层模型.
    with tf.name_scope('input'):  # input/x, input/y
        x = tf.placeholder(tf.float32, [None, 784], name='x-input')
        y_ = tf.placeholder(tf.float32, [None, 10], name='y-input')

    with tf.name_scope('input_reshape'):
        image_shaped_input = tf.reshape(x, [-1, 28, 28, 1])
        # max_output = 10: TensorBoard-IMAGES输出图片数量为10
    ...
    # 对Tensor可视化进行汇总操作（均值、标准差、最大值、最小值、直方图）
    ...
    返回一个和'input'有相同shape和内容的tensor.
    ...
    with tf.name_scope('cross_entropy'):
        ...
        #                               reduction_indices=[1]))
        #
        # 上面这个计算交叉熵损失的公式是不稳定的（numerically unstable）.
        #
        # 所以在这里用tf.nn.softmax_cross_entropy_with_logits来计算,
        # 然后求平均值.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='simple_MNIST',
                        help='Summarise log directory.')
